...
the_viewcount |>
  pivot_longer(cols = 1:5,
               names_to = "a",
               values_to = "viewcount") |>
  select(viewcount) -> viewcount_list
...
the_subscribe |>
  pivot_longer(cols = 1:5,
               names_to = "a",
               values_to = "sub_count") |>
  select(sub_count) -> subscribecount_list
```
...
the_videocount |>
  pivot_longer(cols = 1:5,
               names_to = "a",
               values_to = "video_count") |>
  select(video_count) -> videocount_list
```


```{r}
example <- cbind(title_list, country_list, subscribecount_list, viewcount_list, videocount_list)
example |>
  mutate(viewcount = as.double(viewcount),
         sub_count = as.numeric(sub_count),
         video_count = as.double(video_count),
         view_per_sub = viewcount/sub_count,
         view_per_video = viewcount/video_count) -> final_data
```


------------------------------------------------------------------

get data_set as a list
...
```{r}
map(.x = vector, .f = info) -> test1
```


```{r}
test11 <- tibble(a = test1)
...
test11 |>
  ...
  hoist(statistics, 
        ...
        video = c("videoCount")) |>
  select(title, country, view, sub, video) -> test111
